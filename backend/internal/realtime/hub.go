...
func (h *Hub) Run(ctx context.Context) {
	for {
		...
		case client := <-h.register:
			...
			h.mu.Unlock()
			log.Printf("WebSocket client connected to poll live stream: %s (Active listeners: %d)", client.PollID, len(h.clients[client.PollID]))
		...
		case client := <-h.unregister:
			...
			h.mu.Unlock()
			log.Printf("WebSocket client disconnected from poll stream: %s", client.PollID)
		}
	}
}
